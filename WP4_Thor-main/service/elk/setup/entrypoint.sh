...
# --------------------------------------------------------
# Create TLS certificates for master node and possibly any number of data nodes
...
if [ ! -f config/certs/certs.zip ]; then
  echo "Creating certs";
  echo -ne "instances:\n  - name: $ES_MASTER_NODE_NAME-0\n" > config/certs/instances.yml;
  for (( i=0;i<$ES_DATA_NODE_NUM;i++ ))
  do
    echo -ne "  - name: $ES_DATA_NODE_NAME-$i\n" >> config/certs/instances.yml;
  done
...
users_roles=(
  ...
  [metricbeat_internal]='metricbeat_writer,kibana_admin,ingest_admin,remote_monitoring_agent,remote_monitoring_collector'
  [filebeat_internal]='filebeat_writer,kibana_admin,ingest_admin'
)
